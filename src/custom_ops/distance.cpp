...
void mapTensor2Dblocks(poplar::Graph &graph, poplar::Tensor &t) {
  ...
  auto blockSize0 = std::max<unsigned>(
      std::ceil(t.dim(0) / nTiles),
  ...
  for (auto i = 0u; i < nBlocks0; ++i) {
    for (auto j = 0u; j < nBlocks1; ++j) {
      ...
      graph.setTileMapping(
          t.slice({std::min<unsigned>(i * blockSize0, t.dim(0)),
...
poplar::Tensor
getCachedCopy(std::map<std::pair<size_t, size_t>, poplar::Tensor> &cache,
              poplar::Graph &graph, size_t tile, size_t index,
...
poplar::Tensor l1distance(poplar::Graph &graph, const poplar::Tensor &a,
  ...
  if (a.rank() != 2 || b.rank() != 2 || a.dim(1) != b.dim(1)) {
    ...
    msg << "Bad arguments to l1distance, expected a.shape (M, K), b.shape (N, "
           "K), actual"
        << " a.shape = " << a.shapeToString()
        << ", b.shape = " << b.shapeToString() << ".";
    throw std::invalid_argument(msg.str());
  }
  const size_t n = b.dim(0);
  poplar::Tensor out = graph.addVariable(a.elementType(), {a.dim(0), b.dim(0)},
                                         {debugContext, "l1dist_out"});
  ...
  const auto vertexName =
      poputil::templateVertex("L1DistanceSingleVertex", a.elementType());
...
poplar::Tensor l1distancegrad(poplar::Graph &graph, const poplar::Tensor &a,
                              ...
                              poplar::program::Sequence &prog,
                              const poplar::DebugContext &debugContext) {
  if (a.rank() != 2 || b.rank() != 2 || gradOutput.rank() != 2 ||
      a.dim(1) != b.dim(1) || gradOutput.dim(0) != a.dim(0) ||
      gradOutput.dim(1) != b.dim(0)) {
    std::ostringstream msg;
  ...
  poplar::Tensor grad = graph.addVariable(a.elementType(), a.shape(),
                                          {debugContext, "l1dist_grad"});
  ...
  const auto vertexName =
      poputil::templateVertex("L1DistanceGradSingleVertex", a.elementType());
  ...
  for (size_t i = 0; i < mapping.size(); ++i) {
    for (const auto &interval : mapping[i]) {
      for (size_t j = interval.begin(); j != interval.end(); ++j) {
        ...
        graph.connect(
            v["b"],
            getCachedCopy(bCache, graph, i, a2_index,
                          b.slice({a2_index, a2_index + 1}, 1).squeeze({1}),
                          prog));
        graph.connect(v["gradOutput"],
                      getCachedCopy(gradCache, graph, i, a1_index,
...
poplar::Tensor l2distance(poplar::Graph &graph, const poplar::Tensor &a,
  ...
  const size_t n = b.dim(0);
  poplar::Tensor out = graph.addVariable(a.elementType(), {a.dim(0), b.dim(0)},
                                         {debugContext, "l2dist_out"});
  mapTensor2Dblocks(graph, out);
  const auto &mapping = graph.getTileMapping(out);
  poplar::ComputeSet cs = graph.addComputeSet({debugContext, "l2dist"});
  const auto vertexName =
      poputil::templateVertex("L2DistanceSingleVertex", a.elementType());
  for (size_t i = 0; i < mapping.size(); ++i) {
    for (const auto &interval : mapping[i]) {
      for (size_t j = interval.begin(); j != interval.end(); ++j) {
        size_t a_index = j / n;
...
poplar::Tensor l2distancegrad(poplar::Graph &graph, const poplar::Tensor &a,
                              ...
                              poplar::program::Sequence &prog,
                              const poplar::DebugContext &debugContext) {
  if (a.rank() != 2 || b.rank() != 2 || dist.rank() != 2 ||
      gradOutput.rank() != 2 || a.dim(1) != b.dim(1) ||
  ...
  const size_t k = a.dim(1);
  poplar::Tensor grad = graph.addVariable(a.elementType(), a.shape(),
                                          {debugContext, "l2dist_grad"});
  mapTensor2Dblocks(graph, grad);
  const auto &mapping = graph.getTileMapping(grad);
  poplar::ComputeSet cs = graph.addComputeSet({debugContext, "l2dist_grad"});
  const auto vertexName =
      poputil::templateVertex("L2DistanceGradSingleVertex", a.elementType());
  for (size_t i = 0; i < mapping.size(); ++i) {
    for (const auto &interval : mapping[i]) {
      for (size_t j = interval.begin(); j != interval.end(); ++j) {
        size_t a1_index = j / k;
