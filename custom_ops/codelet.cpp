...
float l1dist(const float *a, const float *b, size_t size) {
  ...
  if (size % 2)
    res += ipu::fabs(a[size - 1] - b[size - 1]);
  return res;
}
...
template <typename T> float l1dist(const T *a, const T *b, size_t size) {
  float sum = 0.0;
...
template <typename T> class L1DistanceSingleVertex : public poplar::Vertex {
public:
...
template <typename T> static inline T signum(T x) {
...
static inline half4 signum(half4 x) {
  ...
  const uint32_t pZeroOrOne[2] = {pXNonzero[0] & pOne[0],
                                  pXNonzero[1] & pOne[1]};
...
float l1distgrad(const float &a, const float *b, const float *grad,
                 size_t size) {
  float2 sum = {0.0, 0.0};
...
template <typename T> class L1DistanceGradSingleVertex : public poplar::Vertex {
public:
  poplar::Input<T> a;
...
template <typename T> float l2dist(const T *a, const T *b, size_t size) {
  float sum = 0.0;
  for (size_t i = 0; i < size; ++i) {
    float diff = float(a[i] - b[i]);
    sum += diff * diff;
...
template <typename T> class L2DistanceSingleVertex : public poplar::Vertex {
public:
  poplar::Input<poplar::Vector<T, SPAN, 8>> a;
  poplar::Input<poplar::Vector<T, ONE_PTR, 8>> b;
  poplar::Output<T> out;
...
template <typename T>
float l2distgrad(const T &a, const T *b, const T *dist, const T *grad,
                 size_t size) {
  float sum = 0.0;
  for (size_t i = 0; i < size; ++i) {
...
template <typename T> class L2DistanceGradSingleVertex : public poplar::Vertex {
public:
  poplar::Input<T> a;
  poplar::Input<poplar::Vector<T, SPAN, 8>> b;
